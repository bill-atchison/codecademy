package com.codecademy.services;

import com.codecademy.models.Comment;
import com.codecademy.models.Post;
...
public class PostService {
    ...
    public static void savePost(Post post) {
        String SQL_savePost = "INSERT INTO " + DatabaseInfo.Tables.POSTS +
                        " VALUES (" +
                        "\"" + post.getIdString() + "\"," +
                        "\"" + post.getTitle() + "\", " +
                        "\"" + post.getMessage() + "\", " +
        ...
        Statement statement = connection.createStatement()) {
            ...
            System.out.println("Error saving their post to the database.");
    ...
    public static List<Post> getPostsByUser(User user) {
        String SQL_postByUser = "SELECT * FROM " + DatabaseInfo.Tables.POSTS +
        ...
        try (Connection connection = DriverManager.getConnection(DatabaseInfo.DB_URL);
             Statement statement = connection.createStatement();
             ResultSet results = statement.executeQuery(SQL_postByUser)) {

            while (results.next()) {
                Post post = new Post(results.getString(1), results.getString(2),
                        results.getString(3), user, results.getLong(5), null);

                usersStories.add(post);
            }

        } catch (SQLException e) {
            System.out.println("Failing to retrieve the user’s posts.");
            System.out.println(e.getMessage());
        }
    ...
    public static List<Post> mostRecentNews() {
        String SQL_recentNews = "SELECT * FROM " + DatabaseInfo.Tables.POSTS +
                " ORDER BY POST_DATE LIMIT 5";
